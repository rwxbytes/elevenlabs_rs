...
/// Gets a list of all available voices for a user.
///
/// This is the v1 endpoint, which returns the full list without pagination.
/// Prefer [`GetVoices`] (the paginated v2 endpoint) for large voice libraries.
///
/// # Example
/// ```no_run
/// use elevenlabs_rs::{ElevenLabsClient, Result};
/// use elevenlabs_rs::endpoints::admin::voice::{ListVoices, ListVoicesQuery};
///
/// #[tokio::main]
/// async fn main() -> Result<()> {
///     let c = ElevenLabsClient::from_env()?;
///     let endpoint = ListVoices::with_query(ListVoicesQuery::default().with_show_legacy(true));
///     let voices = c.hit(endpoint).await?;
///     for voice in &voices {
...
/// }
/// ```
/// See the [Get Voices API reference](https://elevenlabs.io/docs/api-reference/voices/get-all-v1)
#[derive(Clone, Debug, Default)]
pub struct ListVoices {
    query: Option<ListVoicesQuery>,
}

impl ListVoices {
    pub fn with_query(query: ListVoicesQuery) -> Self {
...
impl ListVoicesQuery {
    /// If set to true, legacy premade voices will be included in the response.
    pub fn with_show_legacy(mut self, show_legacy: bool) -> Self {
        self.params.push(("show_legacy", show_legacy.to_string()));
        self
    }
}

impl crate::endpoints::sealed::Sealed for ListVoices {}

impl ElevenLabsEndpoint for ListVoices {
    const PATH: &'static str = "/v1/voices";

    const METHOD: Method = Method::GET;

    type ResponseBody = ListVoicesResponse;
...
/// List voices response body (v1).
#[derive(Clone, Debug, Deserialize)]
pub struct ListVoicesResponse {
    pub voices: Vec<GetVoiceResponse>,
}

impl<'a> IntoIterator for &'a ListVoicesResponse {
...
impl IntoIterator for ListVoicesResponse {
