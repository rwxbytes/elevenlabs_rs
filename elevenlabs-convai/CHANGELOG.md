...
- **Breaking**: `AgentWebSocket::start_conversation` now returns `AgentWebSocketSession` instead of an anonymous stream
- **Breaking**: `AgentWebSocket` implementation fields are now private; use constructors, builder methods, and read-only accessors instead
- **Breaking**: `writer_task_tx` now uses a bounded `tokio::sync::mpsc::Sender`
